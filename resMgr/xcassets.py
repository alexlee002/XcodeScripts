...
class ImageModel(object):
	...
	def equalsTo(self, other, ignoreName=False):
		if not type(other) is ImageModel:
			return False
		if self.scale == other.scale and self.idiom == other.scale and self.subtype == others.subtype:
			if ignoreName:
				return True
	...
	def parseImageFileName(fileName):
 	...
 	def imageModeFromDict(dic):
 		...
 		(model.idiom, fn, model.scale, model.subtype, model.resizing) = valueOrNoneFromDictWithKeys(dic, ('idiom', 'filename', 'scale', 'subtype', 'resizing'))
...
class XCAssets(object):
	...
	def buildImagesets(self, fastMode=True):
			...
			for f in imageFiles:
				...
				if imgModel.sliceImage:
					...
					if not resizingImage and not originalJson:
						...
						if not originalInfo or not originalInfo.has_key('resizing'):
							Logger().error('Lost information for image:"%s". This image should be a resizable image, but we could not find any information from the original Contents.json, please check it manually.' % imgModel.filePath)
							sys.exit(1)
		...
		def _findImageInfoFromOriginalJson(imageModel, originalJson):
			if not type(originalJson) is dict or not originalJson.has_key('images'):
...
class ResizingImage(object):
	...
	def parseResizingImage(self):
		...
		if self.vLoc == None and self.hLoc == None:
			return None
